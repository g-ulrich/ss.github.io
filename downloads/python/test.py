...
r.login('gabe.ulrich@yahoo.com', 'r478dfg61dsQ')
...
def close_prices(n):
    prices = 'close_price'
    stock_data = r.get_historicals(STOCK, span='year', bounds='regular')
    return loopStockInfo(prices, stock_data)[-n:]
        ...
        if result == "buy" or result == "sell":
            try:
                ...
                if buy_or_sell != "":
                    ...
                    if buy_or_sell == "sell":
                        # if current_shares >= 1.0:
                        r.orders.order_sell_market('NIO', current_shares, timeInForce='gfd', extendedHours='false')
                        # else:
                        #     pass
                    elif buy_or_sell == "buy":
                        r.orders.order_buy_market('NIO', shares_to_buy, timeInForce='gfd', extendedHours='false')
                    else:
                        print("Error in market_order() neither buy or sell.")
